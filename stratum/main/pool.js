...
const Pool = function (config, configMain, callback) {
  ...
  this.primary = {
    payments: {
      enabled:
        _this.config.primary.payments && _this.config.primary.payments.enabled,
    },
    zmq: {
      enabled: _this.config.primary.zmq && _this.config.primary.zmq.enabled,
    },
  };
  this.auxiliary = {
    ...
    payments: {
      enabled:
        ...
        _this.config.auxiliary.payments &&
        _this.config.auxiliary.payments.enabled,
    },
    zmq: {
      enabled:
        _this.config.auxiliary &&
        _this.config.auxiliary.zmq &&
  ...
  this.authorizeWorker = function (
    ...
    _this.checkPrimaryWorker(
      ip,
      port,
      addrPrimary,
      () => {
        _this.checkAuxiliaryWorker(
          ip,
          port,
          addrAuxiliary,
          (authAuxiliary) => {
  ...
  this.checkWorker = function (daemon, address, callback) {
    daemon.sendCommands(
      ...
      (results) => {
        callback(
          results.filter((result) => !result.error && result.response.isvalid)
            .length > 0,
        );
      },
    );
  ...
  this.checkPrimaryWorker = function (
    ip,
    port,
    address,
    callback,
  ...
  this.checkAuxiliaryWorker = function (
    ip,
    port,
    address,
    callback,
  ...
  this.checkAccepted = function (daemon, hash, callback) {
    daemon.sendCommands([['getblock', [hash]]], false, (results) => {
      const blocks = results.filter((result) => {
        return (
          result.response &&
          result.response.hash === hash &&
          result.response.confirmations >= 0
        );
      });
  ...
  this.checkAuxiliary = function (shareData, auxShareData) {
    if (_this.auxiliary.enabled) {
      ...
      const shareDiff =
        Algorithms.sha256d.diff / Number(_this.auxiliary.rpcData.target);
  ...
  this.checkDownloaded = function (daemon) {
    daemon.sendCommands([['getblockchaininfo', []]], false, (results) => {
      ...
      daemon.sendCommands([['getpeerinfo', []]], true, (result) => {
        ...
        const totalBlocks = peersBlocks.reduce((max, cur) =>
          max >= cur ? max : cur,
        );
  ...
  this.handleValidation = function (block, workers, sending) {
    ...
    const transactionFee = _this.config.primary.payments
      ? _this.config.primary.payments.transactionFee
      : 0;
    ...
    workers.forEach((worker) => {
      if (worker.miner in combined) {
        combined[worker.miner].times = Math.max(
          worker.times,
          combined[worker.miner].times,
        );
    ...
    const maxTime = Math.max(
      ...Object.keys(combined).map((worker) => combined[worker].times),
    );
  ...
  this.handlePrimaryTemplate = function (newBlock, newBroadcast, callback) {
    ...
    _this.primary.daemon.sendCommands(commands, true, (result) => {
      if (result.error) {
        ...
        try {
          const newBlockFound = _this.manager.handleTemplate(
            result.response,
            newBlock,
            newBroadcast,
          );
  ...
  this.handlePrimaryPayments = function (payments, callback) {
    ...
    Object.keys(amounts).forEach((address) => {
      if (
        amounts[address] < _this.config.primary.payments.minPayment ||
      ...
      if (error) _this.emitLog('error', false, response);
        ...
        _this.emitLog(
          ...
          _this.text.stratumBlocksText7(
            ...
            _this.auxiliary.rpcData.height,
          ),
        );
        _this.checkAccepted(
          _this.auxiliary.daemon,
          _this.auxiliary.rpcData.hash,
          (accepted, transaction) => {
            shareData.transaction = transaction;
            callback(accepted, shareData);
          },
    ...
    if (_this.auxiliary.enabled) {
      _this.auxiliary.daemon.sendCommands(commands, true, (result) => {
        if (result.error) {
          ...
          !_this.auxiliary.height ||
          !_this.auxiliary.previousblockhash
        ) {
      ...
      _this.auxiliary.daemon.sendCommands(commands, true, (result) => {
        if (result.error) {
          ...
          const update =
            ...
            _this.auxiliary.rpcData.hash != result.response.hash;
  ...
  this.submitAuxiliary = function (shareData, hexData, callback) {
    ...
    // Build Daemon Commands
    const auxProof = Buffer.concat([
      utils.varIntBuffer(0),
      utils.packInt32LE(0),
    ]);
  ...
  this.handleAuxiliaryPayments = function (payments, callback) {
    ...
    Object.keys(amounts).forEach((address) => {
      if (
        amounts[address] < _this.config.auxiliary.payments.minPayment ||
  ...
  this.setupAuxiliaryDaemons = function (callback) {
    // Load Daemons from Configuration
    const auxiliaryDaemons = _this.auxiliary.enabled
      ? _this.config.auxiliary.daemons
      : [];
  ...
  this.setupPorts = function () {
    ...
    _this.config.ports.forEach((port, idx) => {
      ...
      if (port.port in _this.difficulty)
  ...
  this.setupBlockPolling = function (callback) {
    ...
    if (!_this.auxiliary.enabled && _this.primary.zmq.enabled) {
      ...
      _this.auxiliary.enabled &&
      _this.primary.zmq.enabled &&
